...
const Reference = ({item, idx}) => {
    return (
        <div className={`reference flex w-[350px] sm:w-[300px] flex-col gap-3 justify-start p-6 pt-0 lg:p-2 fade-in`}>
...
const ReferenceCarousel = ({ items = REFS }) => {
    ...
    return (
        <div id="listRowContainer" className="flex flex-row center justify-center w-full gap-5 h-full transition overflow-hidden relative">
            <button onClick={handlePreviousClick} className="scale-1 transition hover:scale-125 z-10 self-end">←</button>
            ...
            <button onClick={handleNextClick} className="scale-1 transition hover:scale-125 z-10 self-end">→</button>
        </div>
    );
}
